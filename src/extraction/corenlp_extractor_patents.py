...
def get_args():
    ...
    parser.add_argument("--dataset_dump_dir", type=str, default='../../dataset/computer_science/', help="")
    parser.add_argument("--dygiepp_output_dump_dir", type=str, default='../../outputs/dygiepp_output/', help="")
    parser.add_argument("--output_dir", type=str, default='../../outputs/extracted_triples/', help="")
    return parser.parse_args()


args = get_args()
dataset_dump_dir = args.dataset_dump_dir
dygiepp_output_dump_dir = args.dygiepp_output_dump_dir
...
def getDependencyTriples(corenlp_out, dygiepp, cso_topics):
    ...
    for i in range(len(corenlp_out['sentences'])):
        ...
        for dep in dependencies:
            ...
            g.add_node(governor_token_number, postag=tokens[governor_token_number - 1]['pos'],
                       text=tokens[governor_token_number - 1]['lemma'])
            g.add_node(dependent_token_number, postag=tokens[dependent_token_number - 1]['pos'],
                       text=tokens[governor_token_number - 1]['lemma'])
...
def manageEntitiesAndDygieepRelations(dygiepp, cso_topics):
    ...
    for sentence in dygiepp:
        ok_relations += [(mapEntityAcronyms(acronyms, s), p, mapEntityAcronyms(acronyms, o)) for (s, p, o) in
                         dygiepp[sentence]['relations']]
...
def extraction(filename):
    ...
    paper2metadata = {}
    with open(dataset_dump_dir + filename, 'r') as f:
        id = filename.split(".txt")[0]
        lines = f.readlines()
        text = ("".join(lines)).encode('utf8', 'ignore').decode('ascii', 'ignore')
        paper2metadata[id] = {"text": text}
        ...
        if paper_id in paper2dygiepp:
            ...
            try:
                if 'cso_semantic_topics' not in paper2metadata[paper_id].keys():
                    result = cc.run({
                        "text": paper2metadata[paper_id]['text']
                    })
                    paper2metadata[paper_id]['cso_semantic_topics'] = result['semantic']
                    paper2metadata[paper_id]['cso_syntactic_topics'] = result['syntactic']

                corenlp_out = json.loads(nlp.annotate(paper2metadata[paper_id]['text'], properties=props))
                openie_triples = getOpenieTriples(corenlp_out, paper2dygiepp[paper_id],
                                                  paper2metadata[paper_id]['cso_semantic_topics'] +
                                                  paper2metadata[paper_id]['cso_syntactic_topics'])
                pos_triples = getPosTriples(corenlp_out, paper2dygiepp[paper_id],
                                            paper2metadata[paper_id]['cso_semantic_topics'] + paper2metadata[paper_id][
                                                'cso_syntactic_topics'])
                dependency_triples = getDependencyTriples(corenlp_out, paper2dygiepp[paper_id],
                                                          paper2metadata[paper_id]['cso_semantic_topics'] +
                                                          paper2metadata[paper_id]['cso_syntactic_topics'])
                entities, dygiepp_triples = manageEntitiesAndDygieepRelations(paper2dygiepp[paper_id],
                                                                              paper2metadata[paper_id][
                                                                                  'cso_semantic_topics'] +
                                                                              paper2metadata[paper_id][
                                                                                  'cso_syntactic_topics'])
